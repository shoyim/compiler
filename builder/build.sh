...
cleanup(){
    echo "Exiting..."
    docker stop builder_compiler_instance && docker rm builder_compiler_instance
}

fetch_packages(){
    local port=$((5535 + $RANDOM % 60000))
    mkdir build
    # Start a compiler container
    docker run \
        --privileged \
        -v "$PWD/build":'/compiler/packages' \
        -dit \
        -p $port:2000 \
        --name builder_compiler_instance \
        ghcr.io/shoyim/compiler
...
echo "Start your custom compiler container with"
